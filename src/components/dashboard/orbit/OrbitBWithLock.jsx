...
const OrbitBWithLock = ({ className }) => {
  ...
  useEffect(() => {
    ...
    const unlockedCount = isLocked ? 0 : Math.max(0, Math.min(pairs, unlockedLevels.orbitB ?? 0));
    circles.forEach((circle, index) => {
      const pairIndex = Math.floor(index / 2);
      circle.style.opacity = pairIndex < unlockedCount ? '1' : '0.25';
    });
  }, [isLocked, unlockedLevels.orbitB]);

  return (
    <div ref={containerRef} className={`relative ${className || ''}`}>
      <Orbitb className="w-full h-auto" unlockedLevels={unlockedLevels.orbitB ?? 1} />
      {isLocked ? (
